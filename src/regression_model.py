...
# Configuration Parameters for Frame-Based Processing
...
def process_sessions_frame_based(session_dirs, num_frames, emg_means=None, emg_stds=None, use_existing_scaler=False):
    ...
    - If use_existing_scaler is False:
        ...
        - y: NumPy array of continuous labels.
    ...
    y, group_names = create_regression_labels(fingers_angles_df, FINGER_GROUPS)
    logging.info(f"Labels created. Shape: {y.shape}")
    logging.info(f"Number of skipped samples: {skipped}")

    print_label_distribution_regression(y, group_names, label='Processed Sessions')
...
def print_label_distribution_regression(y, group_names, label='Overall'):
    """
    Print statistical distribution of regression labels.

    Parameters:
    - y: NumPy array of regression labels.
    - group_names: List of group names.
    - label: Descriptor for the distribution.
    """
    print(f"\nLabel distribution ({label}):")
    for i, finger_group in enumerate(group_names):
        mean = np.mean(y[:, i])
        std = np.std(y[:, i])
        print(f"{finger_group}: Mean = {mean:.2f}, Std = {std:.2f}")

def evaluate_model_regression(model, X, y, group_names, label='Test Set'):
    """
    Evaluate the regression model and print regression metrics.
    ...
    logging.info(f"{label} MSE: {mse:.4f}")
    logging.info(f"{label} MAE: {mae:.4f}")
    logging.info(f"{label} R²: {r2:.4f}")

    # Print per-group metrics
    for i, finger_group in enumerate(group_names):
        mse_fg = mean_squared_error(y[:, i], y_pred[:, i])
        mae_fg = mean_absolute_error(y[:, i], y_pred[:, i])
        r2_fg = r2_score(y[:, i], y_pred[:, i])
        print(f"\n{finger_group} - {label} Metrics:")
        print(f"MSE: {mse_fg:.4f}, MAE: {mae_fg:.4f}, R²: {r2_fg:.4f}")
...
def main():
    ...
    print_label_distribution_regression(y_train_full, group_names, label='Training Set')
    print_label_distribution_regression(y_val, group_names, label='Validation Set')

    # Build the CNN regression model
    input_shape = (NUM_FRAMES, len(SELECTED_CHANNELS))
    output_dim = y_train_full.shape[1]
    model = build_cnn_model_regression(input_shape, output_dim)
    logging.info("CNN Regression Model built.")
    ...
    evaluate_model_regression(model, X_val_scaled, y_val, group_names, label='Validation Set')

    # Save the model and scaler
    timestamp = pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')
    model_dir = f'model_regression_{timestamp}'
    ...
    evaluate_model_regression(model, X_test_scaled, y_test, group_names, label='Additional Test Set')
